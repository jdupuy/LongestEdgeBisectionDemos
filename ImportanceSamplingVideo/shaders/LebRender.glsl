...
void main()
{
    ...
    float E1 = texture(u_DensitySampler, vec3(i_TexCoord, u_FrameID)).r;
    float E2 = texture(u_DensitySampler, vec3(i_TexCoord, u_FrameID)).g;
    float Var = clamp(E2 - E1 * E1, 0.0f, 1.0f);
    vec3 texel = texture(u_ImageSampler, vec3(i_TexCoord, u_FrameID)).rgb;
