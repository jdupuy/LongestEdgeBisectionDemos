...
void configureTerrainProgram(GLuint glp, GLuint offset)
{
    ...
    GLint locations[] = {TEXTURE_COUNT,
                         ...
                         TEXTURE_COUNT + 2};
    glProgramUniform1iv(
        glp, glGetUniformLocation(glp, "tt_Textures[0]"),
        tt_TexturesPerPage(g_terrain.texture.tt),
        locations
    );
}
...
bool loadTerrainProgram(GLuint *glp, const char *flag, GLuint uniformOffset)
{
    ...
    djgp_push_string(djp, "#define LEB_BUFFER_COUNT 2\n");
    djgp_push_string(djp, "#define TT_LEB_ID 1\n");
    djgp_push_string(djp, "#define TT_TEXTURES_PER_PAGE %i\n", tt_TexturesPerPage(g_terrain.texture.tt));
    djgp_push_string(djp, "#define TT_BUFFER_BINDING_INDIRECTION %i\n", BUFFER_COUNT + 4);
...
// -----------------------------------------------------------------------------
/**
 * Load the TeraTexture
 *
 * This loads a virtual texture
 */
bool loadTeraTexture()
{
    GLenum textureUnits[] = {
        GL_TEXTURE0 + TEXTURE_COUNT,
        GL_TEXTURE1 + TEXTURE_COUNT,
...
bool loadTerrainVariables()
{
    ...
    // update tt_Texture args
    tt_UpdateArgs *args = &g_terrain.texture.args;
    float aspect = float(VIEWER_DEFAULT_WIDTH) / float(VIEWER_DEFAULT_HEIGHT);
    memcpy(args->matrices.modelView, &variables.modelViewMatrix, sizeof(variables.modelViewMatrix));
    memcpy(args->matrices.modelViewProjection, &variables.modelViewProjectionMatrix, sizeof(variables.modelViewProjectionMatrix));
    args->projection = TT_PROJECTION_PERSPECTIVE;
    args->worldSpaceImagePlaneAtUnitDepth.width = 2.0f * tan(radians(g_camera.fovy) / 2.0f) * aspect;
    args->worldSpaceImagePlaneAtUnitDepth.height = 2.0f * tan(radians(g_camera.fovy) / 2.0f);
    args->framebuffer.width  = VIEWER_DEFAULT_WIDTH;
    args->framebuffer.height = VIEWER_DEFAULT_HEIGHT;

    if (!g_terrain.flags.freeze)
        tt_Update(g_terrain.texture.tt, args);
...
void lebRenderCs()
{
    ...
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                     BUFFER_LEB + 1,
                     tt_LebBuffer(g_terrain.texture.tt));
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                     BUFFER_COUNT + 4,
                     tt_IndirectionBuffer(g_terrain.texture.tt));
    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, g_gl.buffers[BUFFER_TERRAIN_DRAW_CS]);
    glBindVertexArray(g_gl.vertexArrays[VERTEXARRAY_MESHLET]);
    ...
    // reset GL state
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, BUFFER_LEB_NODE_BUFFER, 0);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, BUFFER_LEB + 1, 0);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, BUFFER_COUNT, 0);
    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, 0);
    glBindVertexArray(0);
    glDisable(GL_CULL_FACE);
    glDisable(GL_DEPTH_TEST);
...
void mouseMotionCallback(GLFWwindow* window, double x, double y)
{
    ...
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT) == GLFW_PRESS) {
        ...
        dja::mat3 axis = dja::transpose(g_camera.axis);
        g_camera.pos -= axis[2] * dx * 5e-3 * norm(g_camera.pos);
    }
